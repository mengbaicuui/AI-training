...
return answer`
  ...
  },
  {
    ...
    content: [
      ...
      {
        type: 'highlight',
        value: '**什么是 Personalized PageRank (PPR)？** 想象你站在图中的某个节点上，然后开始"随机游走"——每一步，你有一定概率沿着边走到邻居节点，也有一定概率（叫 teleport probability，通常 ~15%）跳回起点。重复无数次后，每个节点被你"踩到"的频率就是它的 PPR 分数。起点附近、连接紧密的节点分数高，远端弱连接的节点分数低。**通俗比喻**：PPR 就像从你家出发逛街——你大概率在家附近的几条街上转悠，偶尔坐公交去远处，但最终还是会回家。那些你经常路过的店（节点）就是 PPR 认为对你最重要的。在 RAG 里，"家"就是查询相关的种子实体，PPR 帮你找到图中与它们关系最紧密的一批节点。'
      }
    ]
  },
  {
    ...
    content: [
      ...
      {
        type: 'table',
        title: 'HippoRAG (v1/v2) vs LinearRAG：关键差异',
        columns: ['特性', 'HippoRAG / HippoRAG2', 'LinearRAG'],
        rows: [
          ['图构建方式', '关系抽取（Relation Extraction）：从文本中提取"主体-谓语-客体"三元组来构建知识图谱', '无关系（Relation-free）层级图（Tri-Graph）：仅建模实体、句子和篇章的包含关系，不抽取谓语'],
          ['PPR 应用阶段', '作为核心检索步骤，使用查询中的概念作为种子节点，直接在 KG 上做 PPR 找到相关子图', '属于第二阶段检索（全局重要性聚合）：先做本地语义桥接，再用 PPR 在 Tri-Graph 上扩散'],
          ['种子节点来源', '直接用从查询中提取的实体/概念作为 PPR 初始种子', '使用第一阶段"本地语义桥接"后产生的激活实体（Activated Entities）作为种子'],
          ['效率与成本', '关系抽取需大量 LLM 调用，索引成本高且易产生噪声', '索引过程不消耗 Token，通过轻量级实体链接实现，具备线性扩展性'],
        ],
      },
    ]
  },
